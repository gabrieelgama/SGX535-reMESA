...
**CONFIRMED [P3-033]** — The Poulsbo DDK uses a nominal 200000000 clock. Linux obtains core_freq via a PCI configuration transaction on the host bridge: it writes selector 0xd0050300 to 0xd0, reads 0xd4 and decodes clock&7 into 100/133/150/178/200/266. Sources: [PINFO:46-51](poulsbo-data/../archaeology-data/PINFO.txt); [CLOCK:11-51](poulsbo-data/CLOCK.txt).
...
**CONFIRMED [P3-037]** — The DDK associates SGX535 121 revision with BRN22934/23944/23410 and 126 with BRN22934. The reset has branch BRN23944 which pauses BIF/limpa fault and a fault drain with temporary PD before restoring the context. Sources: [ERRATA:152-178](poulsbo-data/../archaeology-data/ERRATA.txt); [RESET:471-495](poulsbo-data/../archaeology-data/RESET.txt); [RESET:544-655](poulsbo-data/../archaeology-data/RESET.txt).
...
**INFERRED:** the current probe path already touches the SGX; carregar/descarregar gma500 is not a passive experiment. The operation that Linux calls getting the clock also writes to PCI (P3-033); it should not enter read-only inventory. The constant 200 MHz does not replace detection nor authorize PLL programming.
