...
|---|---|---|
| Inventory without new accesses | board/environment and logs not yet provided | already existing records of PCI, driver, kernel/BIOS, exported resources, and boot |
...
| Rendering | previous ones more streams/ISA/layouts/state/synchronization/isolation | new engineering phase, out of scope |
...
## Minimum proposed experiment — Phase 4 update
...
It is not necessary to resolve 3D streams/textures to prepare inventory. However, to execute any payload, source, license, revision, layout, and stop mechanism need to be checked beforehand.
