# Security audit of register readings
...
| `0x0080` | `SOFT_RESET` | controls SGX resets | Poulsbo | Linux `psb_reg.h:49-56`; `psb_drv.c:103-124` | read used as posted read | write changes multiple blocks | UNKNOWN | gma500 init | action CONFIRMED | UNSAFE |
| `0x0110` | `EVENT_HOST_ENABLE2` | SGX2 IRQ mask | Poulsbo | Linux `psb_reg.h:60-65`; `psb_irq.c:250-305` | readback post-write | run with IRQ/mask | SGX IRQ active | `irqmask_lock` | use CONFIRMED | UNSAFE |
| `0x0114` | `EVENT_HOST_CLEAR2` | acknowledge IRQ SGX2 | Poulsbo | Linux `psb_reg.h:64-65`; `psb_irq.c:192-195` | only post-write readback observed | write-to-clear; read effect UNKNOWN | SGX IRQ active | IRQ handler | use CONFIRMED | UNSAFE |
...
| `0x0130` | `EVENT_HOST_ENABLE` | SGX IRQ mask | Poulsbo | Linux `psb_reg.h:69`; `psb_irq.c:250-305` | readback post-write | concurrency with IRQ | SGX IRQ active | `irqmask_lock` | use CONFIRMED | UNSAFE |
| `0x0134` | `EVENT_HOST_CLEAR` | acknowledge SGX IRQ | Poulsbo | Linux `psb_reg.h:71-84`; `psb_irq.c:192-195` | undocumented read | write-to-clear | SGX IRQ active | IRQ handler | use CONFIRMED | UNSAFE |
| `0x0ab8` | `PDS_EXEC_BASE` | PDS base | Poulsbo | Linux `psb_reg.h:116-118`; `psb_drv.c:361` | not located | execution/address control | UNKNOWN | gma500 init | offset/use CONFIRMED | UNSAFE |
| `0x0ac4` | `EVENT_KICKER` | kick address | Poulsbo | Linux `psb_reg.h:120-121` | not located | associated with submission | UNKNOWN | historical ABI | offset CONFIRMED | UNSAFE |
| `0x0ac8` | `EVENT_KICK` | triggers event | Poulsbo | Linux `psb_reg.h:123-124` | not located | write starts action | UNKNOWN | firmware/command path | offset CONFIRMED | UNSAFE |
| `0x0c00` | `BIF_CTRL` | fault clear/cache/TLB controls | Poulsbo | Linux `psb_reg.h:128-130`; `mmu.c:69-120` | RMW/readback used | flush/invalidateidate/clear fault on write; read side UNKNOWN | UNKNOWN | `mmu->sem` | use CONFIRMED | UNSAFE |
| `0x0c38` | `BIF_DIR_LIST_BASE1` | base/contexto MMU | Poulsbo | Linux `psb_reg.h:126`; `mmu.c:123-135` | not located individually | write changes address space | UNKNOWN | `mmu->sem` | offset CONFIRMED | UNSAFE |
| `0x0c04` | `BIF_INT_STAT` | tipo/requester of the failure | Poulsbo historical/Linux | Linux `psb_reg.h:133-147`; `psb_irq.c:159-188` | read in fault IRQ | clear/latch UNKNOWN | SGX IRQ active | IRQ handler | use CONFIRMED | UNKNOWN |
...
| `0x0000` | `CLKGATECTL` | clock gate | Poulsbo | Linux `psb_reg.h:13-29`; `psb_device.c:85-94` | RMW/readback | write altera gating; read effect UNKNOWN | it is the clock control itself | gma500 PM | use CONFIRMED | UNSAFE |
